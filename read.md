# AquaSentry - Water Quality Prediction Dashboard

## Overview

AquaSentry is an AI-powered water quality prediction and monitoring dashboard built for the IBM Z Datathon 2025. The system integrates satellite imagery analysis, IoT sensor networks, and community health reporting to predict waterborne disease risks and contamination events in real-time.

**Core Purpose:** Provide proactive water safety monitoring by correlating multiple data sources (satellite, IoT sensors, health reports) to predict contamination risks and prevent waterborne disease outbreaks.
...
**October 11, 2025 - Major Feature Enhancements:**
- ✅ **Water Data Loading:** Fixed CSV data loading with success/error messaging and priority ordering
- ✅ **Email Alerts:** Implemented real-time email alerts via SendGrid integration with HTML templates
- ✅ **Historical Trends:** Added comprehensive historical trends visualization with date range filtering
- ✅ **Smart Disease Sorting:** Enhanced disease prediction with ML-based priority sorting (severity + transmission + risk)
- ✅ **Pollution Maps:** Created interactive pollution hotspot maps with geographic visualization
- ✅ **Data Export:** Implemented export functionality supporting CSV, JSON, and Excel formats
- ✅ **Bug Fixes:** Resolved IoT forecasting timestamp compatibility issues
...
- Simulated IBM Z15 mainframe metrics in sidebar
- IBM Db2 for z/OS database references (architectural placeholder)
- IBM MQ message queuing concepts (not actively implemented)
- IBM CICS transaction processing references (conceptual)

**Note:** The IBM Z integration is currently simulated with metrics displays. Full production deployment would integrate with actual IBM Z infrastructure via REST APIs or direct database connections.

### Environment Variables
- `SENDGRID_API_KEY` - SendGrid API key for email alerts (optional)
- `HEALTH_AUTHORITY_EMAIL` - Recipient email for health alerts (optional, defaults to 'health@example.com')